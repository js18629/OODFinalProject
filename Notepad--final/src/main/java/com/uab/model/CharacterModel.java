...
public class CharacterModel extends Glyph {
	...
	public void draw(Graphics g, Point position, int width, int height) {
		g.drawString(Character.toString(this.character), position.getX(), position.getY());

		FontMetrics fm = g.getFontMetrics();
		bounds = new Rect(position.getX(), position.getY() - getHeigth(), 10, 20);
		position.incrementXBy(getWidth());
		if (toUnderline) {
			g.setColor(Color.RED);
			g.drawLine(bounds.getLeft(), bounds.getBottom() + bounds.getHeight(), bounds.getLeft() + bounds.getWidth(),
					bounds.getBottom() + bounds.getHeight());
			g.setColor(Color.BLACK);
		}
		if (this.hasCursor) {
			if (before)
				g.fillRect(bounds.getLeft() - 3, bounds.getBottom(), 2, bounds.getHeight());
			else
				g.fillRect(bounds.getLeft() + bounds.getWidth() - 3, bounds.getBottom(), 2, bounds.getHeight());
		}
